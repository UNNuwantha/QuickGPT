...
const Community = () => {
  ...
  if (loading) return <div className='p-6 pt-12 text-center text-gray-700 dark:text-purple-100'>Loading community images...</div>
      ...
      {images.length > 0 ? (
        <div className='flex flex-wrap max-sm:justify-center gap-5'>
           {images.map((item, index)=>(
            <a key={index} href={item.imageUrl} target='_blank' rel='noreferrer noopener' className='relative group block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-shadow duration-300'>
              <img src={item.imageUrl} alt="Published community image" className='w-full h-40 md:h-50 2xl:h-62 object-cover group-hover:scale-105 transition-transform duration-300 ease-in-out' />
